...
import { questionsPart2, ANSWER_TIME_PART2, dialogueAudio } from '@/data/questionsPart2';
...
const AUDIO_DURATIONS: Record<string, number> = {
  [dialogueAudio]: 68.7,       // dialogue_68 61.5s + 0.5s + q6 6.5s + 0.1s
  [questionsPart2[1].audio]: 5.9,   // q7 实际 5.8s + 0.1s
  [questionsPart2[2].audio]: 5.8,   // q8 实际 5.7s + 0.1s
};
...
export default function Part2Screen({ onSubmit }: Part2ScreenProps) {
  ...
  const [qIndex, setQIndex] = useState(0); // 0=第6题, 1=第14题, 2=第15题
  const [count, setCount] = useState(ANSWER_TIME_PART2);
  ...
  const currentQ = questionsPart2[qIndex];
  ...
  useEffect(() => {
    ...
    setPhase('playing_dialogue_q6');
  }, [phase]);

  useEffect(() => {
    if (phase !== 'playing_dialogue_q6') return;
    startPlayback(dialogueAudio, AUDIO_DURATIONS[dialogueAudio], () => {
      setPhase('countdown_q6');
    });
    // 预加载 q7 和 q8
    preloadAudio(questionsPart2[1].audio);
    preloadAudio(questionsPart2[2].audio);
    return () => cleanup();
  }, [phase, startPlayback, cleanup, preloadAudio]);

  useEffect(() => {
    const playPhases: Phase[] = ['playing_q7', 'playing_q8'];
    if (!playPhases.includes(phase)) return;

    const idx = phase === 'playing_q7' ? 1 : 2;
    const q = questionsPart2[idx];
    const duration = AUDIO_DURATIONS[q.audio];
    const countdownPhase: Phase = phase === 'playing_q7' ? 'countdown_q7' : 'countdown_q8';
  ...
  // ===== 倒计时 =====
  useEffect(() => {
    const countdownPhases: Phase[] = ['countdown_q6', 'countdown_q7', 'countdown_q8'];
    if (!countdownPhases.includes(phase)) return;

    setCount(ANSWER_TIME_PART2);
  ...
  useEffect(() => {
    ...
    if (phase === 'countdown_q6') {
      setQIndex(1);
      setSelected(null);
      selectedRef.current = null;
      setPhase('playing_q7');
    } else if (phase === 'countdown_q7') {
      setQIndex(2);
      setSelected(null);
      selectedRef.current = null;
      setPhase('playing_q8');
  ...
  useEffect(() => {
    ...
    const resultAnswers = questionsPart2.map((qq) => {
    ...
    onSubmit({
      totalQuestions: questionsPart2.length,
      correctCount,
      score: Math.round((correctCount / questionsPart2.length) * 100),
  ...
  // ===== UI =====
  const isDialoguePhase = phase === 'playing_dialogue_q6';
  ...
  const statusText = () => {
    if (isDialoguePhase) {
      switch (audioStatus) {
        ...
        case 'playing': return { icon: <Volume2 className="w-4 h-4 animate-pulse" />, text: '正在播放共用对话...', sub: '第6题选项已显示，可先浏览' };
    ...
    if (phase === 'playing_q7' || phase === 'playing_q8') {
      switch (audioStatus) {
        ...
        <div className="max-w-lg w-full space-y-6">
          {isDialoguePhase && audioStatus === 'playing' && (
            <div className="text-center space-y-1">
              <span className="text-sky-400 text-sm font-medium">共用对话（第6-8题）</span>
              <span className="text-amber-400 text-sm ml-2">第6题选项已显示，可先浏览</span>
            </div>
          )}

...
import { questionsPart4, ANSWER_TIME_PART4, dialogueAudio } from '@/data/questionsPart4';
...
const AUDIO_DURATIONS: Record<string, number> = {
  [dialogueAudio]: 96.5,       // intro 7.0s + dialogue_only 83.4s + silence 0.5s + q13 5.5s + 0.1s
  [questionsPart4[1].audio]: 5.4,   // q14 实际 5.3s + 0.1s
  [questionsPart4[2].audio]: 5.1,  // q15 实际 5.0s + 0.1s
};
...
export default function Part4Screen({ onSubmit }: Part4ScreenProps) {
  ...
  const [count, setCount] = useState(ANSWER_TIME_PART4);
  ...
  const currentQ = questionsPart4[qIndex];
  ...
  useEffect(() => {
    ...
    // 预加载 q14 和 q15
    preloadAudio(questionsPart4[1].audio);
    preloadAudio(questionsPart4[2].audio);
    return () => cleanup();
  ...
  useEffect(() => {
    ...
    const resultAnswers = questionsPart4.map((qq) => {
    ...
    onSubmit({
      totalQuestions: questionsPart4.length,
      correctCount,
      score: Math.round((correctCount / questionsPart4.length) * 100),
